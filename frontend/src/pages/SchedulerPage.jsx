...
const SchedulerPage = () => {
  ...
  const [mode, setMode] = useState('') // 'scheduler' or 'events'
  const [courseEvents, setCourseEvents] = useState([])
  const [selectedCoursesWithSections, setSelectedCoursesWithSections] = useState([])
  ...
  // NEW: Function to fetch course events for specific sections
  const fetchCourseEvents = async () => {
    if (selectedCoursesWithSections.length === 0) {
      setMessage({ type: 'error', text: 'Please add courses with sections first' })
      return
    }

    setIsLoading(true)
    try {
      const response = await fetch(`${BACKEND_API_URL}/scheduler-api/course_events_schedule/`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-CSRFToken': getCsrfToken() || ''
        ...
        body: JSON.stringify({
          sections: selectedCoursesWithSections.map(course => ({
      ...
      setMessage({ type: 'success', text: `Found events for ${Object.keys(data).length} course sections` })
      
      setTimeout(() => setMessage({ type: '', text: '' }), 3000)
    } catch (error) {
      console.error('Error fetching course events:', error)
      setMessage({ type: 'error', text: 'Failed to fetch course events. Please try again.' })
    } finally {
      setIsLoading(false)
    }
  }

  // NEW: Function to export events to calendar
  const exportEventsToCalendar = async () => {
    ...
    try {
      setIsLoading(true)
      const response = await fetch(`${BACKEND_API_URL}/scheduler-api/export_events/`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-CSRFToken': getCsrfToken() || ''
      ...
      setMessage({ type: 'success', text: 'Events exported successfully! Check your downloads.' })
      setTimeout(() => setMessage({ type: '', text: '' }), 3000)
    } catch (error) {
      console.error('Error exporting events:', error)
      setMessage({ type: 'error', text: 'Failed to export events. Please try again.' })
    } finally {
      setIsLoading(false)
    }
  }

  // MODIFIED: Updated addCourse function to handle both modes
  const addCourse = async () => {
    ...
    // For events mode, section is required
    if (mode === 'events' && !newCourse.course_section) {
      setMessage({ type: 'error', text: 'Please select a section for events mode' })
      return
    }
    ...
    if (mode === 'scheduler') {
      ...
      // Events mode - check for exact match including section
      const fullCourseKey = `${courseKey}*${newCourse.course_section}`
      if (selectedCoursesWithSections.some(c => 
        `${c.course_type}*${c.course_code}*${c.course_section}` === fullCourseKey)) {
        setMessage({ type: 'error', text: 'Course section already added' })
        return
      }

      setSelectedCoursesWithSections(prev => [...prev, {
        course_type: newCourse.course_type,
        course_code: newCourse.course_code,
  ...
  // NEW: Remove course for events mode
  const removeCourseWithSection = (index) => {
    setSelectedCoursesWithSections(prev => prev.filter((_, i) => i !== index))
    setCourseEvents([])
  }
  ...
  // MODIFIED: Reset function to handle mode changes
  const resetAllData = () => {
    setSelectedCourses([])
    setSelectedCoursesWithSections([])
    setSchedules([])
    setCourseEvents([])
    setCurrentScheduleIndex(0)
    setPagination({ offset: 0, limit: 50, hasMore: false, total: 'unknown', currentBatch: 0 })
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: '#F3F9FF' }}>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          >
            ...
            <p className="text-xl text-neutral-600 max-w-3xl mx-auto leading-relaxed">
              {mode === '' ? 
                'Choose your preferred mode to get started with course planning.' :
                mode === 'scheduler' ? 
                'Create conflict-free course schedules effortlessly. Select your courses and let our AI find the perfect timetable for you.' :
                'Get detailed course events and deadlines. Add them to your personal calendar to stay organized.'
              }
            </p>
          </motion.div>
          ...
          {mode === '' && (
            ...
            >
              <motion.div
                className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg p-8 border border-white/30 cursor-pointer hover:shadow-xl transition-all duration-300"
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.98 }}
                onClick={() => handleModeChange('scheduler')}
              >
                <div className="text-center">
                  <div className="w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4 shadow-lg" style={{ backgroundColor: '#456882' }}>
                    <Grid className="w-8 h-8 text-white" />
                  </div>
                  <h3 className="text-2xl font-bold mb-4" style={{ color: '#456882' }}>
                    Build Conflict-Free Schedule
                  </h3>
                  <p className="text-neutral-600 mb-6">
                    Generate optimal course schedules without time conflicts. Perfect for planning your semester.
                  </p>
                  <ul className="text-sm text-neutral-600 space-y-2 text-left">
                    ...
                    <li className="flex items-center space-x-2">
                      <CheckCircle className="w-4 h-4 text-green-500" />
                      <span>Multiple schedule options</span>
                    </li>
                    <li className="flex items-center space-x-2">
                      <CheckCircle className="w-4 h-4 text-green-500" />
              ...
              >
                <div className="text-center">
                  ...
                  <p className="text-neutral-600 mb-6">
                    View assignments, labs, and deadlines for your enrolled courses. Export to your calendar.
                  </p>
                  <ul className="text-sm text-neutral-600 space-y-2 text-left">
                    <li className="flex items-center space-x-2">
                      <CheckCircle className="w-4 h-4 text-green-500" />
                      <span>Assignment due dates</span>
                    </li>
                    <li className="flex items-center space-x-2">
                      <CheckCircle className="w-4 h-4 text-green-500" />
                      <span>Lab sessions & exams</span>
                    </li>
                    <li className="flex items-center space-x-2">
                      <CheckCircle className="w-4 h-4 text-green-500" />
                      <span>Calendar export (.ics)</span>
                    </li>
                  </ul>
                </div>
              </motion.div>
          ...
          {/* NEW: Mode indicator and back button */}
          {mode !== '' && (
            <motion.div
            ...
            >
              <div className="flex items-center space-x-3">
                <button
                  ...
                  className="flex items-center space-x-2 px-4 py-2 rounded-lg border border-neutral-300 hover:bg-neutral-50 transition-colors"
                >
                  <ArrowLeft className="w-4 h-4" />
                  <span>Back to Mode Selection</span>
                </button>
                <div className="flex items-center space-x-2 px-4 py-2 rounded-lg" style={{ backgroundColor: '#456882', color: 'white' }}>
                  {mode === 'scheduler' ? <Grid className="w-4 h-4" /> : <Calendar className="w-4 h-4" />}
                  <span className="font-medium">
                    {mode === 'scheduler' ? 'Schedule Builder' : 'Course Events'}
                  </span>
                </div>
              </div>
            </motion.div>
          ...
          {/* How to Use - Updated for both modes */}
          {mode !== '' && (
            <motion.div
            ...
            >
              <div className="flex items-center mb-6">
                ...
                <h2 className="text-2xl font-bold" style={{ color: '#456882' }}>
                  How to Use {mode === 'scheduler' ? 'Schedule Builder' : 'Course Events'}
                </h2>
              </div>
              ...
              <div className="grid md:grid-cols-3 gap-6">
                <div className="text-center">
                  ...
                  <h3 className="font-semibold mb-2" style={{ color: '#456882' }}>
                    {mode === 'scheduler' ? 'Select Term & Courses' : 'Select Term & Course Sections'}
                  </h3>
                  <p className="text-sm text-neutral-600">
                    {mode === 'scheduler' ? 
                      'Choose your academic term and add the courses you want to take. Optionally select specific sections.' :
                      'Choose your academic term and add the specific course sections you are enrolled in.'
                    }
                  </p>
                </div>
                ...
                <div className="text-center">
                  ...
                  <h3 className="font-semibold mb-2" style={{ color: '#456882' }}>
                    {mode === 'scheduler' ? 'Generate Schedules' : 'Fetch Events'}
                  </h3>
                  <p className="text-sm text-neutral-600">
                    {mode === 'scheduler' ? 
                      'Our algorithm finds all possible conflict-free schedule combinations.' :
                      'Get all assignments, labs, exams and their due dates for your course sections.'
                    }
                  </p>
                </div>
                ...
                <div className="text-center">
                  ...
                  <h3 className="font-semibold mb-2" style={{ color: '#456882' }}>
                    {mode === 'scheduler' ? 'Browse & Select' : 'Export & Import'}
                  </h3>
                  <p className="text-sm text-neutral-600">
                    {mode === 'scheduler' ? 
                      'Browse through generated schedules and find the one that works best for you.' :
                      'Export events to calendar format and import into Google Calendar, Outlook, or Apple Calendar.'
                    }
                  </p>
                </div>
              </div>
      ...
      {/* Main Content - Show only when mode is selected */}
      {mode !== '' && (
        <section className="pb-20 px-4">
          <div className="max-w-6xl mx-auto">
            ...
            {/* Course Selection - Modified for both modes */}
            <motion.div
              className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg p-8 mb-8 border border-white/30"
              initial={{ opacity: 0, y: 30 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              <h2 className="text-2xl font-bold mb-6" style={{ color: '#456882' }}>
                {mode === 'scheduler' ? 'Add Courses' : 'Add Course Sections'}
              </h2>
              ...
              <div className="grid md:grid-cols-5 gap-4 mb-6">
                ...
                >
                  <option value="">
                    {mode === 'scheduler' ? 'Any Section' : 'Select Section'}
                  </option>
                ...
                <motion.button
                  onClick={addCourse}
                  disabled={!newCourse.course_type || !newCourse.course_code || (mode === 'events' && !newCourse.course_section)}
              ...
              {mode === 'events' && (
                <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                  <p className="text-sm text-blue-800">
                    ...
                    For course events, you must select specific sections as events are section-specific.
                  </p>
                </div>
              )}

              {/* Selected Courses - Different display for each mode */}
              {((mode === 'scheduler' && selectedCourses.length > 0) || 
                (mode === 'events' && selectedCoursesWithSections.length > 0)) && (
                <div>
                  <h3 className="text-lg font-semibold mb-4" style={{ color: '#456882' }}>
                    {mode === 'scheduler' ? 
                      `Selected Courses (${selectedCourses.length})` : 
                      `Selected Course Sections (${selectedCoursesWithSections.length})`
                    }
                  </h3>
                  <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {(mode === 'scheduler' ? selectedCourses : selectedCoursesWithSections).map((course, index) => (
                      <motion.div
                        key={index}
                      ...
                      >
                        ...
                        <button
                          onClick={() => mode === 'scheduler' ? removeCourse(index) : removeCourseWithSection(index)}
            ...
            {((mode === 'scheduler' && selectedCourses.length > 0) || 
              ...
              >
                <motion.button
                  onClick={mode === 'scheduler' ? () => generateSchedules(false) : fetchCourseEvents}
                  disabled={isLoading}
                ...
                >
                  ...
                  <span>
                    {isLoading ? 
                      (mode === 'scheduler' ? 'Generating...' : 'Fetching...') : 
                      (mode === 'scheduler' ? 'Generate Schedules' : 'Get Course Events')
                    }
                  </span>
                </motion.button>
              </motion.div>
            ...
            {/* EXISTING: Schedule Results for scheduler mode */}
            {mode === 'scheduler' && schedules.length > 0 && (
              <motion.div
                ref={resultsRef}
                className="space-y-8"
                initial={{ opacity: 0, y: 30 }}
              ...
              >
                ...
                {/* Schedule Grid Display - Updated without export buttons */}
                    ...
                    <div key={rowIndex} className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
                      {schedules.slice(rowIndex * 3, (rowIndex + 1) * 3).map((schedule, scheduleIndex) => {
                        ...
                        return (
                          ...
                          >
                            {/* Enhanced Schedule Header - Export button removed */}
            ...
            {/* NEW: Course Events Results for events mode */}
            {mode === 'events' && Object.keys(courseEvents).length > 0 && (
              <motion.div
                className="space-y-8"
                initial={{ opacity: 0, y: 30 }}
                animate={{ opacity: 1, y: 0 }}
              ...
              >
                ...
                <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg p-6 border border-white/30">
                  <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
                    <div>
                      <h3 className="text-xl font-bold mb-2" style={{ color: '#456882' }}>
                        Course Events & Deadlines
                      </h3>
                      <div className="flex flex-wrap items-center gap-4 text-sm text-neutral-600">
                        <div className="flex items-center space-x-2">
                          <Calendar className="w-4 h-4" />
                          <span>{Object.keys(courseEvents).length} course section{Object.keys(courseEvents).length > 1 ? 's' : ''}</span>
                        </div>
                        <div className="flex items-center space-x-2">
                          <Hash className="w-4 h-4" />
                          <span>
                            {Object.values(courseEvents).reduce((total, events) => total + events.length, 0)} total events
                          </span>
                        </div>
                      </div>
                    </div>
                ...
                <div className="space-y-6">
                  {Object.entries(courseEvents).map(([courseKey, events]) => (
                    <div key={courseKey} className="bg-white/80 backdrop-blur-sm rounded-xl shadow-lg border border-white/30 overflow-hidden">
                      <div className="p-4 border-b border-neutral-200" style={{ backgroundColor: '#456882' }}>
                      ...
                      <div className="p-6">
                        {events.length === 0 ? (
                          <p className="text-neutral-600 text-center py-8">
                            No events found for this course section.
                          </p>
                        ) : (
                          <div className="space-y-4">
                            {events.map((event, idx) => (
                              <div key={idx} className="flex items-start space-x-4 p-4 bg-neutral-50 rounded-lg hover:bg-neutral-100 transition-colors">
                                <div className="w-2 h-2 rounded-full mt-3" style={{ backgroundColor: '#456882' }}></div>
                                <div className="flex-1">
                                  <div className="flex items-start justify-between mb-2">
                                    <h5 className="font-semibold text-lg" style={{ color: '#456882' }}>
                                      {event.event_type}
                                    </h5>
                                    {event.weightage && (
      ...
      {/* Full Schedule View Modal - Export button removed from modal header */}
      <AnimatePresence>
        {selectedScheduleForView && (
          ...
          >
            ...
            >
              {/* Modal Header - Export button removed */}
...
// Full Schedule View Component for Modal - Export functionality removed
            ...
            <div key={courseKey} className="flex items-center space-x-3">
              ...
              <div>
                ...
                <div className="text-xs text-neutral-600">
                  {events.length} session{events.length > 1 ? 's' : ''} per week
                </div>
              </div>
            </div>
          ))}
